...
public sealed class ExportService : IExportService
{
    ...
    public ExportService(IAppLogger logger)
    {
        _logger = logger;
    }
    ...
    public async Task ExportToCsvAsync(ScanResult result, string filePath)
    {
        ...
        _logger.LogInfo($"Exported {result.LargestFiles.Count} files to CSV: {filePath}");
    }

    public async Task ExportFilesToCsvAsync(IEnumerable<FileSystemItem> files, string filePath)
    {
        var sb = new StringBuilder();
        ...
        await File.WriteAllTextAsync(filePath, sb.ToString());
        _logger.LogInfo($"Exported {rank - 1} files to CSV: {filePath}");
    }
    ...
    public async Task ExportToJsonAsync(ScanResult result, string filePath)
    {
        ...
        _logger.LogInfo($"Exported scan results to JSON: {filePath}");
    }
